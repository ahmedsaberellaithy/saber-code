...
const { ClaudeInterface } = require('./index');

const program = new Command();
const claude = new ClaudeInterface({ rootPath: process.cwd() });
...
async function initialize() {
  try {
    ...
    console.log(chalk.yellow('⚠️  Could not initialize project context, continuing with basic mode...'));
  }
}
...
program
  .name('saber')
  .description('🤖 AI-powered code assistant with file editing and project analysis')
  .version('1.0.0');
  ...
  .action(async () => {
    try {
      ...
      models.forEach(model => {
        console.log(`  ${chalk.green('•')} ${model.name} ${chalk.gray(`(${model.size})`)}`);
      });
    } catch (error) {
      console.error(chalk.red(`❌ Error: ${error.message}`));
    }
...
function showContext() {
  const context = claude.getContext();
  console.log('\n' + chalk.blue.bold('📁 Project Context:'));
  console.log(`  Project: ${chalk.green(context.projectSummary?.projectName || 'Unknown')}`);
  console.log(`  Files: ${chalk.cyan(context.projectSummary?.fileCount || 0)} total`);
  console.log(`  Loaded: ${chalk.cyan(context.currentFiles?.length || 0)} files in context`);
  ...
  if (context.recentChanges.length > 0) {
    ...
    context.recentChanges.slice(-3).forEach(change => {
      console.log(`    • ${change.operation.filePath} ${chalk.gray(`(${change.operation.operation})`)}`);
    });
  }
  console.log('');
}
...
async function loadFiles(patterns) {
  try {
    ...
    const files = await claude.loadFiles(patternList);
    console.log(chalk.green(`✅ Loaded ${files.length} files into context`));
    
    files.forEach(file => {
      console.log(`   ${chalk.green('•')} ${file.path} ${chalk.gray(`(${file.lines} lines)`)}`);
    });
    console.log('');
  } catch (error) {
...
async function analyzeFile(filePath, model = 'codellama') {
  try {
    ...
    const response = await claude.analyzeCode(filePath, model);
...
async function getProjectSummary(model = 'codellama') {
  try {
    ...
    const response = await claude.getProjectSummary(model);
...
async function applyEdit(description, model = 'codellama') {
  try {
    ...
    const result = await claude.applyCodeChanges(description, model);
...
async function searchCode(term, model = 'codellama') {
  try {
    ...
    const response = await claude.searchCode(term, model);
